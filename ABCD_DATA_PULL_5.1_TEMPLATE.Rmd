...
ABCD_VARS <- unique(c(
  ...
  "rel_family_id", "interview_age", "interview_date", "socialdev_visit_date", 
  "socialdev_visit_date_l",
...
MERGED_ALLTIME_SPECVAR <- dplyr::mutate(MERGED_ALLTIME_SPECVAR, SUBS_BVD = {
  if ("SUBS_BVD" %in% names(MERGED_ALLTIME_SPECVAR)) {
    date <- lubridate::ymd_hm(SUBS_BVD)
    year <- lubridate::year(date)
    month <- lubridate::month(date)
    year + (month - 1) / 12
  } else {
    SUBS_BVD  # Return unchanged if SUBS_BVD not present
  }
})

## FOLLOW-UP VISITS
MERGED_ALLTIME_SPECVAR <- dplyr::mutate(MERGED_ALLTIME_SPECVAR, SUBS_FVD = {
  if ("SUBS_FVD" %in% names(MERGED_ALLTIME_SPECVAR)) {
    date <- lubridate::ymd_hm(SUBS_FVD)
    year <- lubridate::year(date)
    month <- lubridate::month(date)
    year + (month - 1) / 12
  } else {
    SUBS_FVD  # Return unchanged if SUBS_BVD not present
  }
})
