...
if (any(grepl("^mri_y_dti_|^mri_y_rsi_", colnames(MERGED_ALLTIME_SPECVAR)))) {
  ...
  # Apply NA to mri_y_dti_ or mri_y_rsi_ columns (assuming these are the intended targets)
  MERGED_ALLTIME_SPECVAR <- MERGED_ALLTIME_SPECVAR %>%
    mutate(across(
...
MERGED_ALLTIME_SPECVAR <- MERGED_ALLTIME_SPECVAR %>% select(-matches("^imgincl_|^[tr]fmri_.*meanmotion$|^mri_info_deviceserialnumber$"))
...
write.csv(ALL_WAVES_CLEAN_REORDER, "UNPREDICTABILITY_KATIE_4.11.25.csv", row.names=FALSE, na="")

write.csv(CODEBOOK, "CODEBOOK_UNPREDICTABILITY_KATIE_4.11.25.csv", row.names=FALSE, na="")
